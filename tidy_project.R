...
# Step 4 - Labels the data set with descriptive variable names and corrects a typo

names(subject_data) <- "subject"
full_data <- cbind(x_data, y_data, subject_data)

fulldataCols <- colnames(full_data)

fulldataCols <- gsub("BodyBody", "Body", fulldataCols)
fulldataCols <- gsub("^f", "freq", fulldataCols)
fulldataCols <- gsub("^t", "time", fulldataCols)

colnames(full_data) <- fulldataCols
